...
''');(bin_dir/'gh').chmod(0o755)
            ...
            archive=downloads/'ki-varslinger-2.0.1.zip'
            ...
            run=subprocess.run(['bash',str(script),'2.0.1'],env=env,capture_output=True,text=True)
            self.assertEqual(run.returncode,0,run.stdout+run.stderr)
            self.assertEqual(git('--git-dir',str(remote),'show','main:keep.txt').strip(),'Unrelated file must survive.')
            self.assertEqual(git('--git-dir',str(remote),'rev-parse','main').strip(),git('--git-dir',str(remote),'rev-parse','v2.0.1^{}').strip())
            ...
            second=subprocess.run(['bash',str(script),'2.0.1'],env=env,capture_output=True,text=True)
            ...
            third=subprocess.run(['bash',str(script),'2.0.1'],env=env,capture_output=True,text=True)
