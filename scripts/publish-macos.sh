#!/bin/bash
# Bruk: bash publish-macos.sh 1.2.0 [sti/til/ki-varslinger-1.2.0.zip]
# Kjorer pa macOS med Bash 3.2+, Git, GitHub CLI, Python 3 og rsync.
set -euo pipefail

V="${1:-}"
if [[ ! "$V" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  echo "Bruk: bash $0 1.2.0 [sti/til/ki-varslinger-1.2.0.zip]" >&2
  exit 1
fi
...
gh repo edit "$GH_REPO" --enable-issues --description "Home Assistant-varsler for familie, Roborock, alarm og Ruter." --add-topic home-assistant --add-topic hacs --add-topic custom-integration
...
git commit -m "KI Varslinger v$V"
git tag -a "$TAG" -m "KI Varslinger $V"
# Publiser commit og tag samlet. Ingen force-push.
git_gh push --atomic origin main "refs/tags/$TAG"

gh release create "$TAG" "$ZIP" --repo "$GH_REPO" --verify-tag --title "KI Varslinger $V" --notes-file RELEASE.md
